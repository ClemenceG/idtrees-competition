...
#     raster = rasterio.open(ras_path)
    ...
    gdf = pd.read_csv(detection_path)
    gtf = pd.read_csv(
        par.datadir + "submission/" + site + "_ground.csv"
    )
    print()
    print('name', pl)
    print("Number of detections: ", len(gdf))
    # gtf['id'] = gtf.index
...
def run_segmentation_evaluation(par):
    ...
    list_plots = [os.path.basename(x) for x in glob.glob("./RS/RGB/*.jpg")]
    print(list_plots)
...
#         im = from_raster_to_img(im_pt)
        ...
        if os.path.exists(detection_path) == False:
            print()
            print("No submission file found for site: ", site)
            continue
        gdf_limits, gtf_limits, itc_name = get_vertex_per_plot(pl, par)
        if gdf_limits is None: continue
        ...
        for obs_itc in range(gdf_limits.shape[0]):
            ...
            for det_itc in range(gtf_limits.shape[0]):
                ...
                R[obs_itc, det_itc] = RandNeon(obs, preds)
                # calculate the iou
...
#                 RandNeon(obs, preds, im, par, pname=str(i) + "_" + pl)
    ...
    pd.DataFrame(task1_evaluation, columns=["itc_id", "rand_index", "IoU"]).to_csv(
        par.outputdir + "/" + par.outputfn +".csv"
    )
